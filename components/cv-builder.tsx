...
export default function CvBuilder({ profile }: { profile: Profile | null }) {
  ...
  const applyRecordToForm = useCallback((record: CvRecord | null) => {
    ...
    setCustomSections(normalizeCustomSections(record.custom_sections))
  }, [])

  const resetToBlankCv = (defaults?: { certIds: string[] }) => {
    setActiveCvId(null)
  ...
  const loadCvRecords = useCallback(async (profileId: string, defaults?: { certIds: string[] }) => {
    ...
    const mapped: CvRecord[] = (data ?? []).map((row: any) => ({
      ...
      custom_sections: normalizeCustomSections(row.custom_sections),
  ...
  const saveCurrentCv = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        custom_sections: normalizeCustomSections(customSections),
      }
      ...
      if (savedId) {
        ...
        if (fresh) {
          applyRecordToForm({
            ...
            custom_sections: normalizeCustomSections(fresh.custom_sections),
    ...
    <div className="p-4 md:p-8 space-y-6">
      ...
      <div className="flex flex-col xl:flex-row gap-6 items-start">
        ...
        >
          ...
          <div>
            <h3 className="text-sm font-black uppercase tracking-wider text-slate-500 mb-3">Side Section</h3>
            ...
            />
             <label className="flex items-center gap-2">
              ...
              {includeCertificates && (
                <div className="ml-6 space-y-2 max-h-36 overflow-y-auto pr-1">
          ...
          <div className="space-y-3">
            <h3 className="text-sm font-black uppercase tracking-wider text-slate-500">Main content</h3>
            <input
              value={headline}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12">
              <aside className="lg:col-span-4 bg-slate-900 dark:bg-slate-950 text-white p-8 space-y-8">
                ...
                {includeCertificates && selectedCertificates.length > 0 && (
                  <div>
                    <h2 className="text-lg font-semibold mb-4 tracking-wide uppercase">Certificates</h2>
